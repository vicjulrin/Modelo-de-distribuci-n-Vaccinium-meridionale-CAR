...
list_selectvars_model_prel<- pblapply(list_hclust_model_prel, function(y) {
  ...
  var_table <- with(cordend_data$labels, data.frame(y_center = x, y_min= x-0.5, y_max=x+0.5, Var = as.character(label), height = 1)) %>% 
    list(vif_data_h) %>% plyr::join_all() %>%  arrange(group, VIF) %>% mutate(Var = if_else(!duplicated(group), paste("*", Var , sep = ""), Var),
                                                                              colVar = if_else(!duplicated(group), "red", "black") ) %>% 
    arrange(y_center) %>% 
  ...
  if(check_permut){
    var_table<- var_table %>% mutate(Var = if_else(permutation.importance>=1, paste("*", Var , sep = ""), Var),
                                     colVar = if_else(permutation.importance>=1, "red", "black") )
  }
  ...
  if(check_permut){
    labels_importance<- var_table %>% dplyr::mutate(label_imp= paste0("[", round(percent.contribution), ";", round(permutation.importance), "]"), 
                                                    label_imp_col= if_else(permutation.importance>=1, "blue", "black")) 
    
    ggdendroPlot<- ggdendroPlot+
      geom_label(data= labels_importance, aes(y= y_center, x= Inf, label= label_imp),  color= labels_importance$label_imp_col, size= 3, hjust=1.1, fill = var_table$col, label.size = 0)
  }
